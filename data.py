...
def make_country_df(country):
    def make_df(condition):
        ...
        df = (
            df.drop(columns=["Province/State", "Country/Region", "Lat", "Long"]).sum().reset_index(name=condition)
        )
        df = df.renmae(columns={"index":"date"})
        return df 
    ...
    for condition in conditions:
        ...
        if final_df is None:
            ...
            fianl_df = final_df.merge(condition_df)
